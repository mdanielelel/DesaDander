...
class Access extends CommonUtils\Access {
	...
	public function maybeUpdateOptions( $option, $oldValue, $value ) {
		...
		foreach ( $this->roles as $wpRole => $role ) {
			...
			// Update dynamic role.
			if ( $options->accessControl->dynamic->has( $role ) ) {
				$useDefault = true;
				foreach ( $options->accessControl->dynamic->$role->all() as $capability => $enabled ) {
					if ( 'useDefault' === $capability ) {
						continue;
					}
					...
					if ( $useDefault && $enabled !== $options->accessControl->dynamic->$role->getDefault( $capability ) ) {
						$useDefault = false;
					}

					$options->accessControl->dynamic->$role->$capability = $enabled;
				}
				$options->accessControl->dynamic->$role->useDefault = $useDefault;
			}
		}
	...
	public function hasCapability( $capability, $checkRole = null ) {
		...
		foreach ( $this->roles as $wpRole => $role ) {
			...
			} elseif ( $options->accessControl->dynamic->has( $role ) ) { // Check for dynamic role.
				$hasCapability = $options->accessControl->dynamic->{ $role }->{ $capabilityName };
				if ( $options->accessControl->dynamic->$role->useDefault ) {
					$hasCapability = $options->accessControl->dynamic->{ $role }->getDefault( $capabilityName );
				}
			}
	...
	public function canManage( $checkRole = null ) {
		...
		foreach ( $this->roles as $wpRole => $role ) {
			...
			$isCustomRole = aioseo()->options->accessControl->dynamic->has( $role );

			if ( ! aioseo()->options->accessControl->has( $role ) && ! $isCustomRole ) {
				continue;
			}

			$roleSettings = $isCustomRole ? aioseo()->options->accessControl->dynamic->$role->all() : aioseo()->options->accessControl->$role->all();
			...
			if ( true === $roleSettings['useDefault'] ) {
				foreach ( $roleSettings as $capability => $enabled ) {
					$roleSettings[ $capability ] = $isCustomRole ?
						aioseo()->options->accessControl->dynamic->$role->getDefault( $capability ) :
