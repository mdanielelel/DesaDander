<?php
namespace AIOSEO\Plugin\Extend\Utils;
...
class Tags {
	 ...
	 */
	public function replaceTags( $string, $id, $stripPunctuation = false ) {
	...
	public function getTagValue( $tag, $id ) {
		switch ( $tag['id'] ) {
			...
			case 'image_seo_title':
				...
				return ! empty( $metaData->title ) ? aioseo()->meta->title->prepareTitle( $metaData->title ) : '';
			case 'image_seo_description':
				$metaData = aioseo()->meta->metaData->getMetaData( $id );
				return ! empty( $metaData->description ) ? aioseo()->meta->description->prepareDescription( $metaData->description ) : '';
			case 'post_seo_title':
				$postId   = get_the_ID();
				$post     = get_post( $postId );
				$metaData = aioseo()->meta->metaData->getMetaData( $postId );
				return ! empty( $metaData->title ) ? aioseo()->meta->title->prepareTitle( $metaData->title ) : aioseo()->meta->title->prepareTitle( $post->post_title );
